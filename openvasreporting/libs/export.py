...
def export_to_excel_by_host(resulttree: ResultTree, template=None, output_file='openvas_report.xlsx'):
    ...
    for i, key in enumerate(temp_resulttree, 1):
        ...
        ws_toc.write("D{}".format(i+3), resulttree[key].nv['critical'], format_align_border_right)
        ws_toc.write("E{}".format(i+3), resulttree[key].nv['high'], format_align_border_right)
        ws_toc.write("F{}".format(i+3), resulttree[key].nv['medium'], format_align_border_right)
        ws_toc.write("G{}".format(i+3), resulttree[key].nv['low'], format_align_border_right)
        ws_toc.write("H{}".format(i+3), resulttree[key].nv_total(), format_align_border_right)
        ws_toc.write("I{}".format(i+3), resulttree[key].higher_cvss, format_toc[Config.cvss_level(resulttree[key].higher_cvss)])
        ws_toc.set_row(i + 3, __row_height(name, 150), None)
        ...
        ws_host.set_column("D:D", 10, format_align_center) # port.port/port.num
        ws_host.set_column("E:E", 10, format_align_center) # family
        ws_host.set_column("F:F", 22, format_align_center) # description
        ws_host.set_column("G:G", 22, format_align_center) # recomendation (solution)
        ws_host.set_column("H:H", 12, format_align_center) # recomendation type (solution_type)
        ws_host.set_column("I:I", 12, format_align_center)
        ws_host.set_column("J:J", 50, format_align_center)
        ws_host.set_column("K:K", 50, format_align_center)

        
        ws_host.merge_range("B2:K2", 'IP: '+resulttree[key].ip + ' | Hostname: ' + resulttree[key].host_name, format_sheet_title_content)
        ws_host.write('B3', "CVSS", format_table_titles)
        ws_host.write('C3', "Name", format_table_titles)
        ws_host.write('D3', "Port", format_table_titles)
        ws_host.write('E3', "Family", format_table_titles)
        ws_host.write('F3', "Description", format_table_titles)
        ws_host.write('G3', "Recomendation", format_table_titles)
        ws_host.write('H3', "Type of fix", format_table_titles)
        ws_host.write('I3', "CVEs", format_table_titles)
        ws_host.write('J3', "Impact", format_table_titles)
        ws_host.write('K3', "References", format_table_titles)
        ...
        for j, vuln in enumerate(resulttree[key].vuln_list, 4):
            ...
            ws_host.write('D{}'.format(j), portnum + '/' + port.protocol, format_align_border_left)
            ws_host.write('E{}'.format(j), vuln.family, format_align_border_left)
            ws_host.write('F{}'.format(j), vuln.description.replace('\n', ' '), format_align_border_left)
            ws_host.write('G{}'.format(j), vuln.solution.replace('\n', ' '), format_align_border_left)
            ws_host.write('H{}'.format(j), vuln.solution_type, format_align_border_left)

            ws_host.write('I{}'.format(j),str(vuln.cves).replace('[','').replace(']','').replace("\'",'').replace(',',' '), format_align_border_left)

            ws_host.write('J{}'.format(j), str(vuln.impact), format_align_border_left)
            ws_host.write('K{}'.format(j), str(vuln.references).replace('Unknown','').replace('https://','').replace('http://','').replace(',',' | '), format_align_border_left)
